            ...
            <div class="grid grid-cols-12 gap-4 mt-4">
                ...
                <div class="col-span-12 md:col-span-6">
                    <p-card header="{{ 'WORKER.REGISTER' | translate }}">
                        ...
                        <!-- <p *ngIf="waHref()">
                            <a [href]="waHref()!" target="_blank" rel="noopener" class="inline-flex align-items-center gap-1 text-green-500 font-medium mt-2"> <i class="pi pi-whatsapp"></i>{{ 'BOOKING_DETAIL.WA_LINK' | translate }} </a>
                        </p> -->
                ...
                <div class="col-span-12" *ngIf="booking.status === 2 && booking.outstandingAmount > 0">
                    <p-message severity="warn" [text]="'BOOKING_DETAIL.OUTSTANDING_AMOUNT_TITLE' | translate"> </p-message>
                ...
                <p-dialog [header]="'BOOKING_DETAIL.REPLACEMENT_REASON_TITLE' | translate" [(visible)]="showReasonDialog" [modal]="true" [style]="{ width: '28rem' }">
                        ...
                        <p-table [value]="applicants()" *ngIf="applicants().length > 0">
                            ...
                            <ng-template #body let-a>
                                <tr>
                                    ...
                                    <td>
                                        <p-button label="{{ 'BOOKING_DETAIL.SET_AS_REPLACEMENT' | translate }}" size="small" severity="warn" (onClick)="openReasonDialog('applicant', a.id)"></p-button>
                                    </td>
                                </tr>
                            </ng-template>
                        </p-table>
                ...
                <div class="col-span-12" *ngIf="booking.status === 5 && !booking.hasReviewed">
                    <p-card header="{{ 'REVIEW.RATE_WORKER' | translate }}">
                        <div class="flex flex-column gap-3">
                            ...
                            <textarea pInputTextarea [(ngModel)]="reviewComment" rows="3" class="w-full" [placeholder]="'REVIEW.COMMENT' | translate"></textarea>
                ...
                <div class="col-span-12" *ngIf="booking.status === 2 && booking.requirePaymentProof">
                    <p-card header="{{ 'PAYMENT.UPLOAD_PROOF' | translate }}">
                        <form [formGroup]="paymentForm" class="grid grid-cols-12 gap-4">
                            ...
                            <div class="col-span-12">
                                <label class="block font-bold mb-1">{{ 'PAYMENT.UPLOAD_PROOF' | translate }}</label>
                                <p-fileupload name="proofImage" mode="basic" accept="image/*" maxFileSize="5000000" [auto]="false" [chooseLabel]="'BOOKING_DETAIL.CHOOSE_RECEIPT' | translate" (onSelect)="onProofSelected($event)"> </p-fileupload>
                                <span *ngIf="proofFileName" class="text-sm text-muted-color">{{ proofFileName }}</span>
                            </div>
                            <div class="col-span-12 text-center">
                                <p-button [label]="'PAYMENT.UPLOAD_PROOF' | translate" icon="pi pi-upload" (onClick)="submitPaymentProof()" [loading]="isSubmitting" [disabled]="paymentForm.invalid || !proofFile"> </p-button>
                            </div>
                        </form>
                    </p-card>
                </div>
...
export class BookingDetail implements OnInit {
    ...
    private loadBooking(id: number) {
        this.bookingService.getBookingById(id).subscribe({
            next: (data: BookingDetailDto) => {
                ...
                const prefillAmount = data.outstandingAmount > 0 ? data.outstandingAmount : (data.paymentAmount ?? data.commissionAmount);
        ...
        if (this.booking != null)
            this.bookingService.requestReplacement(this.booking.id, reason, null, applicationId).subscribe({
                next: () => {
                    ...
                    if (this.booking != null) this.loadBooking(this.booking.id);
                }
            });
    }
    ...
    canRequestReplacement(): boolean {
        return this.booking !== null && (this.booking.status === 3 || this.booking.status === 4) && this.booking.replacementCount < this.booking.maxReplacement;
    }
    ...
    //     };
    //     return this.translate.instant(keys[b.status] ?? '', { id: b.id, name: b.workerFullName || b.homeownerName, date: new Date(b.startDate).toLocaleDateString() });
    // }

    // waHref(): string | null {
    //     if (!this.booking) return null;
    //     const phone = this.booking.workerWhatsApp || this.booking.workerPhone;
    //     if (!phone) return null;
    //     return waLink(phone, this.waMessage());
    // }
}
